...
namespace FusionRipper.Core.Data.Chunks.ObjectChunks.ObjectCommon
{
    public class ObjectCommon : ObjectInfoProperties
    {
        ...
        public override void ReadCCN(ByteReader reader, params object[] extraInfo)
        {
            ...
            if (FRipCore.iOS && FRipCore.Build >= 284)
            {
            ...
            else if (FRipCore.Android && FRipCore.Build >= 284)
            {
                MovementsOffset = reader.ReadShort();
                AnimationOffset = reader.ReadShort();
                reader.Skip(2);
            ...
            else if (FRipCore.Build == 284 && OrderCheck == 0)
            {
                ValueOffset = reader.ReadShort();
                reader.Skip(4);
                MovementsOffset = reader.ReadShort();
            ...
            if ((FRipCore.iOS && FRipCore.Build >= 284) || FRipCore.HTML)
                reader.Skip(2);
            else if (FRipCore.Android && (FRipCore.Build == 284 || FRipCore.Build == 288))
                AnimationOffset = reader.ReadShort();
            else if (!FRipCore.Android && FRipCore.Build >= 284)
                DataOffset = reader.ReadShort();
            else
